...
public interface CompanyDao {
    ...
    //添加公司信息
    @Insert({"insert into company (gsm,gsjs,tp) values (#{company.gsm},#{company.gsjs},#{company.tp})"})
    Integer insert(@Param("company") Company company);

    //通过id修改公司信息
    @Update({"update company set gsm=#{company.gsm},gsjs=#{company.gsjs},tp=#{company.tp} where id=#{company.id}"})
